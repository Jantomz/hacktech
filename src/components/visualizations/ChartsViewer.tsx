...
export default function ChartsViewer() {
  ...
  const handleDocumentProcessing = async () => {
    try {
      ...
      setBudgetEntries(data.data.budget_entries || []);
    } catch (error: any) {
          ...
          <div className="flex justify-center items-center h-64">
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Make sure your VITE_ORKES_PAT environment variable is set.
              </p>
            </div>
          </div>
        </CardContent>
  ...
  const top10Items = [...budgetEntries]
    ...
    .slice(0, 10)
    .map((entry) => ({
      name: entry.department + (entry.subcategory ? ` - ${entry.subcategory}` : ""),
